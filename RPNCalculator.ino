...
#define INPUT_BUFFER_SIZE 17                                                     // Max characters for input (16 + null terminator)
...
uint16_t thresholds[] = {57, 62, 65, 68, 80, 83, 90, 98, 120, 134, 154, 180, 256, 343, 514, 1024}; // Keypad analog value thresholds
char keys[] = { '+', 'E', '.', '0', '-', '3', '2', '1', '*', '6', '5', '4', '/', '9', '8', '7' };  // Coresponding key
...
char getKey() {
    ...
    if (val < KEYPAD_IDLE_THRESHOLD) return 0;                                   // Return 0 if no key pressed
    val = analogRead(KEYPAD_PIN);                                                // Take second reading for stability
    for (uint8_t i = 0; i < KEYPAD_NUM_KEYS; i++)                                // Find which key was pressed
...
void handleDigit(char key) {
    ...
    if (inputIndex < INPUT_BUFFER_SIZE - 1)                                      // Add digit if space is available
        inputBuffer[inputIndex++] = key, inputBuffer[inputIndex] = '\0';         // Append digit and null-terminate
}
...
void handleDecimal() {
    ...
    if (!strchr(inputBuffer, '.') && inputIndex < INPUT_BUFFER_SIZE - 1)         // Add decimal if none exists
        inputBuffer[inputIndex++] = '.', inputBuffer[inputIndex] = '\0';         // Append dot and null-terminate
}

void handleEnter() {
    if (inputIndex) {                                                            // If there's input, push to stack
        stack.push(atof(inputBuffer));                                           // Convert input string to float and push onto stack
        inputIndex = 0;                                                          // Reset input index
        inputBuffer[0] = '\0';                                                   // Clear input buffer
        newInput = true;                                                         // Mark as ready for new input
        showResult = false;                                                      // Don't show result until next operation
    }
}

void handleOperator(char op) {
    if (inputIndex) handleEnter();                                                // Push current input if any
    if (stack._top < 1) return showError("Need 2 operands");                      // Need at least 2 values
    float b = stack.pop(), a = stack.pop(), r;                                    // Pop operands
    if (op == '/' && b == 0) return stack.push(a), showError("Divide by zero");   // Check division by zero
...
void updateDisplay() {
    ...
    for (int i = 0; i < n; i++) stack.push(temp[i]);                               // Restore stack
